...
setting:	SET STRING settingvalue { setSetting($2, $3); sgFree($2); sgFree($3) } ;
...
destination:	DESTINATION STRING { curDest = newDestList($2); sgFree($2); } ;
...
destination_content:
		...
		| WITHIN STRING   { addDestListTime(curDest, $2, T_WITHIN);  sgFree($2); }
		| OUTSIDE STRING  { addDestListTime(curDest, $2, T_OUTSIDE); sgFree($2); }
...
source:		SOURCE STRING { curSrc = newSourceList($2); sgFree($2); };
...
source_content:	  source_match   { addSourceListMatch(curSrc, $1); }
		| source_static
		| WITHIN STRING  { addSourceListTime(curSrc, $2, T_WITHIN); sgFree($2); }
		| OUTSIDE STRING { addSourceListTime(curSrc, $2, T_OUTSIDE); sgFree($2); }
...
acl:		  STRING { curAcl = newAccessList($1); sgFree($1); } aclwithin
		| ELSE { curAcl = newAccessList(curAcl->name); }   aclwithin
		;

aclwithin:
		| WITHIN STRING  { addAccessListTime(curAcl, $2, T_WITHIN);  sgFree($2); }
		| OUTSIDE STRING { addAccessListTime(curAcl, $2, T_OUTSIDE); sgFree($2); }
		;
...
access_dest:
		...
		| access_dest STRING     { addDestinationAccessCheck(curAcl,0,$2); sgFree($2); }
		| access_dest '!' STRING { addDestinationAccessCheck(curAcl,1,$3); sgFree($3); }
		;

rew:		REWRITE STRING { curExpr = newRewrite($2); sgFree($2); }
		;
...
rew_content:	  SUBST  { addRewriteExpression(curExpr, $1); sgFree($1); }
		| WITHIN STRING { /* sgRewriteTime($2, T_WITHIN); */ sgFree($2); }
		| OUTSIDE STRING { /* sgRewriteTime($2, T_OUTSIDE); */ sgFree($2); }
		;


time:		TIME STRING { curTimeMatch = newTimeMatch($2); sgFree($2); }
		;
...
weekly:		  STRING    { curWM = addWeeklyElement(curTimeMatch, $1); sgFree($1); } wtime
